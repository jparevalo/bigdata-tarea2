...
BUSINESS_CATEGORIES = {}
MAX_REVIEWS_BY_CAT = {}
class TopReviewerByCategoryAndVotes(MRJob):
    def mapper_user_reviews_by_cat_and_votes(self, _, line):
        for l in get_json_element(line):
            if 'categories' not in l:
                total_votes = int(l['useful'] + l['funny'] + l['cool'])
                if l['business_id'] in BUSINESS_CATEGORIES:
                    ...
                    for category in categories:
                        if category not in MAX_REVIEWS_BY_CAT:
                            MAX_REVIEWS_BY_CAT[category] = [0,'none',0]
                        yield [l['user_id']+';'+category, total_votes]
            else:
    ...
    def steps(self):
        return [MRStep(mapper=self.mapper_user_reviews_by_cat_and_votes, reducer=self.reducer),
            MRStep(mapper=self.reduce_max_reviews_by_category)]

if __name__ == '__main__':
    TopReviewerByCategoryAndVotes.run()
